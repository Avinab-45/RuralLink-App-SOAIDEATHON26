...
# Render's filesystem is ephemeral unless a persistent disk is attached.  A
# DATABASE_PATH environment variable lets the deployment use that disk while
# retaining the repository-local database for local development.
DB_PATH = Path(os.environ.get("DATABASE_PATH", str(ROOT / "data" / "s14.sqlite3")))
...
def get_driver_location(driver_id: str) -> dict[str, Any] | None:
    """Return the most recent GPS point sent by a driver."""
    with _connection() as connection:
...
def ensure_driver(driver_id: str) -> None:
    """Create a demo driver on first login without changing existing drivers."""
    with _connection() as connection:
        connection.execute(
            """INSERT OR IGNORE INTO drivers
               (driver_code, name, phone, status)
               VALUES (?, ?, ?, ?)""",
            (driver_id, driver_id, "", "ACTIVE"),
        )

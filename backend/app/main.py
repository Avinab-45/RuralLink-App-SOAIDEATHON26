...
from app.database import (
    ...
    assign_planned_orders_to_drivers,
    get_driver_location,
    ensure_driver,
...
DEPOT_COORDS = (19.8135, 85.8312)  # Puri Central Depot
...
DRIVER_PWA_DIR = Path(__file__).resolve().parents[2] / "frontend" / "dist-driver"
if DRIVER_PWA_DIR.exists():
    app.mount("/assets", StaticFiles(directory=str(DRIVER_PWA_DIR / "assets")), name="driver-assets")

    # Vite builds this project from driver.index.html.  StaticFiles only
    # recognises index.html for a directory request, so explicitly serve the
    # real entry point at /driver/.  Without this, /driver/ can fall through
    # to the customer site (or a stale service-worker response).
    @app.get("/driver", include_in_schema=False)
    @app.get("/driver/", include_in_schema=False)
    def driver_pwa_entry():
        return FileResponse(DRIVER_PWA_DIR / "driver.index.html")
...
def create_order(payload: CustomerOrderRequest):
    ...
    for village_name in KNOWN_VILLAGE_COORDS:
        if village_name in message_lower:
            parsed["village"] = village_name.title()
            break
    ...
    order = StructuredOrder(
        ...
        # Replaced below after coordinates are known.  The ML model must not
        # receive a fixed demo distance for every customer order.
        distance_km=0.0, order_weight_kg=payload.weight_kg,
        customer_priority_flag=int(parsed.get("urgency") in {"high", "emergency"}),
    )
    order = _fill_defaults_from_village(order)
    order.distance_km = max(
        0.1,
        round(_haversine_km(DEPOT_COORDS, (order.latitude, order.longitude)), 2),
    )
    prediction = ml_service.predict_order(order)
    if order.urgency == "emergency":
        prediction["priority_score"] = max(prediction["priority_score"], 95.0)

    elif order.urgency == "high":
        prediction["priority_score"] = max(prediction["priority_score"], 75.0)

    elif order.urgency == "low":
        prediction["priority_score"] = min(prediction["priority_score"], 35.0)
...
def plan_saved_orders(payload: PlanSavedOrdersRequest):
    ...
    assignments = assign_planned_orders_to_drivers(rows, result["vehicle_routes"])
    ...
    return {
        **result,
        ...
        "notes": "Saved orders: parser -> ML -> SQLite -> OR-Tools -> driver assignment -> road route.",
    }
...
def driver_login(credentials: dict):
    driver_id = str(credentials.get("driverId", "")).strip().upper()
    if not driver_id:
        raise HTTPException(status_code=422, detail="Driver ID is required")
    ensure_driver(driver_id)
    return {
...
def driver_route(driver_id: str):
    ...
    deliveries = [
        delivery
        for delivery in list_driver_deliveries(driver_id)
    ...
    if not deliveries:
        return {
            ...
            "coordinates": [],
            "stopCoordinates": [],
            "routingMode": "No active deliveries",
        }
    ...
    points = [DEPOT_COORDS] + [
        (float(delivery["latitude"]), float(delivery["longitude"]))
    ...
    return {
        ...
        "route": [delivery["dropoffPoint"] for delivery in valid_deliveries],
        "coordinates": route_data["coordinates"],
        "stopCoordinates": [
            {"name": "Puri Central Depot", "coordinates": list(DEPOT_COORDS)}
        ] + [
            {
                "name": delivery["dropoffPoint"],
                "coordinates": [delivery["latitude"], delivery["longitude"]],
            }
            for delivery in valid_deliveries
        ],
...
def driver_location_latest(driver_id: str):
    location = get_driver_location(driver_id)
    if location is None:
        raise HTTPException(
            status_code=404,
            detail="No GPS location received from this driver yet",
        )
    return location
...
CUSTOMER_WEB_DIR = Path(__file__).resolve().parents[2] / "frontend"
app.mount(
    "/",
    StaticFiles(directory=str(CUSTOMER_WEB_DIR), html=True),

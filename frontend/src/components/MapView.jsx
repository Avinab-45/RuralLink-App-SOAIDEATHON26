...
const warehouseIcon = L.divIcon({
  ...
  html: `
    <div style="
      ...
      display:flex;align-items:center;justify-content:center;
      font-size:21px;border:3px solid white;
      box-shadow:0 2px 8px rgba(0,0,0,.35);
    ">🏭</div>
  `,
  iconSize: [40, 40],
...
const PURI_DEPOT = [19.8135, 85.8312];
...
function MapView() {
  ...
  style={{ zIndex: 0 }}
        ...
        >
          ...
          {location && (
           <Marker
              position={[location.latitude, location.longitude]} icon={truckIcon}>
  <Popup>
            ...
            (stop, index) => (
              <Marker
                ...
                icon={L.divIcon({
                  ...
                  html: `<div style="width:32px;height:32px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:${index === 0 ? "#0f766e" : "#2563eb"};border:3px solid white;box-shadow:0 3px 10px rgba(0,0,0,.28);color:white;font-weight:800;font-size:13px;">${index === 0 ? "🏭" : index}</div>`,
                  iconSize: [32, 32],
                  iconAnchor: [16, 16],
                })}
              >
                <Popup>
                  <strong>
                    {index === 0 ? "Driver start" : `Delivery ${index}`}: {stop.name}
                  </strong>
                </Popup>
              </Marker>
            )
          )}
        <Marker position={PURI_DEPOT} icon={warehouseIcon}>
  <Popup>Puri warehouse / depot</Popup>
</Marker>
        </MapContainer>
